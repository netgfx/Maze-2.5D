...
export function getLine(proxyCell: { x: any; y: any; left: any; right: any; top: any; bottom: any }, index: any) {
  ...
  if (cell.top) {
    lines.push({ rotation: [0, Math.PI / 2, 0], position: [2.5 + cell.x * 5.0, 0, 0.5 + cell.y * 5] })
  }
  if (cell.bottom) {
    lines.push({ rotation: [0, Math.PI / 2, 0], position: [2.5 + cell.x * 5, 0, 4.5 + cell.y * 5] })
  }
  if (cell.left) {
    lines.push({ rotation: [0, 0, 0], position: [0.5 + cell.x * 5, 0, 2.5 + cell.y * 5] })
  }
  if (cell.right) {
    lines.push({ rotation: [0, 0, 0], position: [4.5 + cell.x * 5, 0, 2.5 + cell.y * 5] })
  }
